#!/usr/bin/env python3
"""
Validation script for Spanish_Example_Words column.
...
def validate_examples():
    csv_file = '/home/user/LPH/SpanishWords/SpanishWordsOverview.csv'
    ...
    with open(csv_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            base_words = parse_array(row.get('Spanish_Base_Words', ''))
            example_words = parse_array(row.get('Spanish_Example_Words', ''))
            combined_words = parse_array(row.get('Spanish_Combined_Words', ''))

            total_packs += 1
            total_base_words += len(base_words)

            # Skip packs without examples (not yet populated)
            ...
            # Check 5: Examples should add meaningful context (at least 2 extra chars for Spanish)
            ...
            for ex in example_words:
                ...
                for i, base_word in enumerate(base_words):
                    if base_word in ex:
                        ...
                        if extra_chars < 2:  # Spanish needs at least 2 extra chars (e.g., "el", "mi")
    ...
    print("=" * 60)
    print("SPANISH EXAMPLE WORDS VALIDATION REPORT")
    print("=" * 60)
    print(f"\nTotal Packs in File: {total_packs}")
    print(f"Packs with Examples (Validated): {validated_packs}")
    print(f"Total Base Words (validated packs): {total_base_words}")
